...
class PHPExcel_Calculation
{
    ...
    {
        if (is_string($value)) {
            $first_val = substr($value, 0, 1);
            if ((isset($first_val)) && ($first_val == '"') && (substr($value, -1) == '"')) {
...
//                    echo 'Token is a PHPExcel constant: '.$excelConstant.'<br />';
                ...
                } elseif ((is_numeric($token)) || ($token === null) || (is_bool($token)) || ($token == '') || (substr($token, 0, 1) == '"') || ($token[0] == '#')) {
